    ...
    return (
      ...
      >
        <div className="flex w-full flex-col items-start gap-4 px-3 py-1">
          {label ? (
            <span className="w-full text-caption-bold font-caption-bold text-secondary-text">
              {label}
            </span>
          ) : null}
        </div>
...
>(function SidebarWithSectionsRoot(
  ...
  return (
    <nav
      className={SubframeUtils.twClassNames(
        "flex h-full w-60 flex-col items-start border-r border-solid border-neutral-border bg-primary-background",
        className
      )}
      ref={ref}
      {...otherProps}
