    ...
    return (
      <div
        className={SubframeUtils.twClassNames(
          "group/2c7966c2 flex w-80 items-center gap-4 rounded-md bg-primary-background px-4 py-3 shadow-lg",
          className
        )}
        ref={ref}
        {...otherProps}
      >
        ...
        <div className="flex grow shrink-0 basis-0 flex-col items-start">
          {title ? (
            <span
              className={SubframeUtils.twClassNames(
                "w-full text-body-bold font-body-bold text-primary-text",
                {
                  "text-success-700": variant === "success",
                  "text-error-700": variant === "error",
          ...
          {description ? (
            <span className="w-full text-caption font-caption text-secondary-text">
              {description}
            </span>
          ) : null}
        </div>
